# Usage

This category is for developers who want to use Bridge in their own scripts.

It focuses on:

- public `FWB.*` namespaces
- exports
- examples
- normal usage patterns

## In This Category

| Page | What It Covers |
|---|---|
| [API Reference](../../api-reference/README.md) | Public runtime modules and namespace usage |
| [Examples](../../examples.md) | Copy-ready examples |

## Good Starting Points

- Start with [API Reference](../../api-reference/README.md) if you want the available public functions.
- Start with [Examples](../../examples.md) if you want something quick to copy.
